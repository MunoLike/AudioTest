...
with open("test.pcm","wb") as f:
    print(len(container.demux(stream)))
    for i, packet in enumerate(container.demux(stream)):
    
        for frame in packet.decode():

            if frame:
                print("saved",len(frame.planes),frame.format.packed.container_name)
                Lbytes = frame.planes[0].to_bytes()
                Rbytes = frame.planes[1].to_bytes()
                for j in range(0,len(Lbytes),4):
                    f.write(Lbytes[j:j+4])
                    f.write(Rbytes[j:j+4])
